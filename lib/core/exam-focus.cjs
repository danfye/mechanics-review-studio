...
const COURSE_PROFILES = [
  ...
  },
  {
    ...
    label: "电路与电子技术",
    match: ["电路", "电工学", "电子技术", "模拟电子", "数字电子", "电路分析", "circuits", "electronics"],
    terms: [
      term("欧姆定律", ["Ohm", "电阻", "电压电流关系", "V=IR"], "dc_circuit", 88),
      term("基尔霍夫定律", ["KCL", "KVL", "节点电流", "回路电压"], "dc_circuit", 90),
      term("节点电压法", ["节点法", "结点电压", "node voltage"], "analysis_method", 84),
      term("网孔电流法", ["网孔法", "回路电流", "mesh current"], "analysis_method", 80),
      term("戴维南等效", ["Thevenin", "诺顿等效", "等效电源"], "equivalent", 86),
      term("RC暂态", ["一阶电路", "时间常数", "充放电"], "transient", 82),
      term("交流相量", ["相量", "阻抗", "RLC", "正弦稳态"], "ac_circuit", 84),
      term("二极管", ["PN结", "整流", "伏安特性"], "electronics", 72),
      term("运算放大器", ["反相放大", "同相放大", "虚短虚断"], "electronics", 76),
    ],
  },
  {
    id: "signals_systems",
    label: "信号与系统",
    match: ["信号与系统", "信号系统", "数字信号处理", "DSP", "signals and systems", "signal processing"],
    terms: [
      term("卷积", ["卷积积分", "卷积和", "convolution"], "time_domain", 88),
      term("冲激响应", ["单位冲激响应", "h(t)", "h[n]"], "system_response", 84),
      term("傅里叶级数", ["周期信号", "谐波", "Fourier series"], "frequency_domain", 78),
      term("傅里叶变换", ["频谱", "FT", "Fourier transform"], "frequency_domain", 88),
      term("拉普拉斯变换", ["s 域", "极点", "零点"], "transform", 84),
      term("Z变换", ["z 域", "离散系统", "差分方程"], "transform", 82),
      term("采样定理", ["Nyquist", "奈奎斯特", "混叠"], "sampling", 86),
      term("系统稳定性", ["BIBO", "极点稳定", "收敛域"], "system_property", 82),
    ],
  },
  {
    id: "automatic_control",
    label: "自动控制原理",
    match: ["自动控制", "控制理论", "控制工程", "控制系统", "automatic control", "control systems"],
    terms: [
      term("传递函数", ["Transfer function", "G(s)", "系统函数"], "model", 88),
      term("方框图化简", ["信号流图", "梅森公式", "闭环传递函数"], "model", 78),
      term("稳定性判据", ["Routh", "劳斯", "Hurwitz", "稳定判据"], "stability", 88),
      term("根轨迹", ["开环极点", "闭环极点", "根轨迹法"], "root_locus", 84),
      term("频率响应", ["Bode", "Nyquist", "幅相特性"], "frequency_response", 86),
      term("稳态误差", ["误差常数", "静态误差", "系统型别"], "performance", 82),
      term("PID控制", ["比例积分微分", "校正", "控制器"], "controller", 74),
      term("状态空间", ["状态方程", "可控性", "可观性"], "state_space", 76),
    ],
  },
  {
    id: "thermo_heat_transfer",
    label: "热力学与传热学",
    match: ["工程热力学", "热力学", "传热学", "热工基础", "thermodynamics", "heat transfer"],
    terms: [
      term("热力学第一定律", ["能量守恒", "闭口系统", "开口系统"], "thermodynamics", 88),
      term("热力学第二定律", ["熵增", "卡诺循环", "不可逆"], "thermodynamics", 86),
      term("理想气体过程", ["等温", "等压", "等容", "绝热过程"], "gas_cycle", 84),
      term("热机循环", ["Otto", "Diesel", "Rankine", "Brayton"], "cycle", 76),
      term("导热", ["傅里叶定律", "热阻", "稳态导热"], "conduction", 88),
      term("对流换热", ["牛顿冷却公式", "换热系数", "Nusselt"], "convection", 84),
      term("辐射换热", ["Stefan-Boltzmann", "黑体", "发射率"], "radiation", 78),
      term("换热器", ["传热单元数", "对数平均温差", "LMTD"], "heat_exchanger", 76),
    ],
  },
  {
    id: "computer_science",
    label: "计算机与数据结构",
    match: ["数据结构", "算法", "计算机组成", "操作系统", "数据库", "离散数学", "computer science", "data structures", "algorithms"],
    terms: [
      term("时间复杂度", ["Big-O", "复杂度分析", "O(n)", "O(log n)"], "algorithm_analysis", 88),
      term("递归", ["递推", "分治", "递归树"], "algorithm_design", 78),
      term("线性表", ["数组", "链表", "顺序表"], "data_structure", 72),
      term("栈和队列", ["栈", "队列", "括号匹配", "BFS"], "data_structure", 80),
      term("树", ["二叉树", "遍历", "堆", "哈夫曼树"], "tree", 84),
      term("图", ["邻接表", "最短路径", "拓扑排序", "最小生成树"], "graph", 86),
      term("排序算法", ["快速排序", "归并排序", "堆排序", "稳定性"], "sorting", 84),
      term("动态规划", ["状态转移", "最优子结构", "背包问题"], "algorithm_design", 86),
      term("关系数据库", ["关系模型", "SQL", "范式", "事务"], "database", 72),
    ],
  },
];

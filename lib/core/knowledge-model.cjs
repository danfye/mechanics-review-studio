...
const { buildCourseLearningPack, buildDocumentLearningPack, learningPackContext } = require("./learning-pack.cjs");
const {
...
function extractFormulaFragments(line) {
  ...
  for (const match of text.matchAll(/(T\s*\(\s*n\s*\)\s*=\s*[^。；;\n]{3,90})/giu)) {
    fragments.push(trimFormulaFragment(match[1]));
  }
  for (const match of text.matchAll(/((?:O|Θ|Omega|\\Omega)\s*\(\s*(?:n|log\s*n|n\s*\^\s*\d+|1)[^)]+\))/giu)) {
    fragments.push(trimFormulaFragment(match[1]));
  }
...
function isUsefulFormulaLine(line) {
  ...
  const hasScienceEquation =
    ...
    /[=≈≤≥]|\//.test(normalized);
  const hasComputationEquation = /T\s*\(\s*n\s*\)\s*=|(?:O|Θ|Omega|\\Omega)\s*\(/i.test(normalized);
  const compactLength = normalized.replace(/\s+/g, "").length;
  if ((!hasOperator && !hasComputationEquation) || (!hasPhysicsSymbol && !hasScienceEquation && !hasComputationEquation) || compactLength > 90) return false;
...
function buildDocumentKnowledgeModel(doc, options = {}) {
  ...
  documentModel.learning_pack = buildDocumentLearningPack(doc, documentModel);
...
function buildCourseKnowledgeModel(course, docs = [], options = {}) {
  ...
  const courseModel = {
    ...
    documents: documentModels.map((doc) => ({
      ...
      learning_pack: doc.learning_pack,
    })),
  ...
  courseModel.learning_pack = buildCourseLearningPack(courseModel);
  return courseModel;
}

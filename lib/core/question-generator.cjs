...
function generateQuestionSet(courseOrModel, docsOrOptions = [], maybeOptions = {}) {
  ...
  candidates.push(...learningPackQuestions(courseModel.learning_pack, { limit: Math.max(8, Math.ceil(count * 0.6)) }).map(makeQuestion));
...
function comprehensiveQuestions(model) {
  ...
  const workflow = examWorkflowForProfile(model.course?.focus_profile?.id);
  ...
  questions.push(makeQuestion({
    ...
    question_text: `综合题：围绕 ${relatedConcepts.slice(0, 4).join("、") || model.course?.name || "本课程"}，设计一道跨知识点题，并写出从${workflow.entry}到${workflow.check}的完整步骤。`,
    answer: `完整答案应包含：${workflow.entry}、关键概念、公式或规则、${workflow.solve}、${workflow.check}。`,
    step_by_step_solution: ["确定跨章节连接点。", "列出每个知识点负责解决的问题。", workflow.solve, `用${workflow.check}做最后校核。`],
...
function examWorkflowForProfile(profileId = "") {
  if (profileId === "advanced_mathematics") {
    return { entry: "条件识别与方法选择", solve: "完成化简、证明或计算", check: "定义域、收敛性、端点和结果形式" };
  }
  if (profileId === "chemistry") {
    return { entry: "反应类型、守恒关系和平衡条件识别", solve: "建立计量、平衡或能量关系并求解", check: "单位、有效数字、近似条件和化学意义" };
  }
  if (profileId === "biology") {
    return { entry: "结构、过程、实验变量或遗传关系识别", solve: "组织机制解释、图表推断或比例计算", check: "对照组、前提条件和生物学解释" };
  }
  if (profileId === "electrical_circuits") {
    return { entry: "参考方向、节点/回路和等效模型识别", solve: "列 KCL/KVL、等效电路或暂态方程", check: "极性、功率守恒和时间常数" };
  }
  if (profileId === "signals_systems") {
    return { entry: "信号类型、系统性质和变换域选择", solve: "建立卷积、频域或变换域关系", check: "收敛域、带宽、因果性和稳定性" };
  }
  if (profileId === "automatic_control") {
    return { entry: "输入输出、闭环结构和性能指标识别", solve: "建立传递函数、特征方程或频域判据", check: "稳定性、稳态误差和动态指标" };
  }
  if (profileId === "thermo_heat_transfer") {
    return { entry: "系统边界、过程类型和传热方式识别", solve: "列能量方程、状态关系或热阻网络", check: "符号约定、单位和过程适用条件" };
  }
  if (profileId === "computer_science") {
    return { entry: "输入规模、数据结构和算法目标识别", solve: "建立状态、递推、遍历或复杂度分析", check: "边界情况、正确性和时间空间复杂度" };
  }
  if (profileId === "college_physics") {
    return { entry: "研究对象、相互作用和守恒/场方程识别", solve: "列运动、守恒、电磁或热学方程", check: "单位、方向、极限情况和物理意义" };
  }
  return { entry: "研究对象、条件和题型入口识别", solve: "建立方程、逻辑链或证明框架", check: "单位、边界条件和结果合理性" };
}
...
function selectDiverseQuestions(candidates, count, model) {
  ...
  const learningPackTarget = Math.min(
    Math.max(1, Math.floor(count * 0.2)),
    deduped.filter(isLearningPackQuestion).length,
  );
  for (const question of deduped.filter(isLearningPackQuestion)) {
    if (selected.length >= count || selected.filter(isLearningPackQuestion).length >= learningPackTarget) break;
    if (!selected.includes(question)) selected.push(question);
  }
...
function isLearningPackQuestion(question) {
  return (question.tags || []).includes("增量知识包");
}
...
function questionSelectionScore(question, model) {
  ...
  const learningPackBoost = isLearningPackQuestion(question) ? 26 : 0;

...
function extractChatCompletionContent(raw) {
  ...
  if (text.startsWith("data:") || text.includes("\ndata:")) {
    ...
    for (const line of text.split(/\r?\n/)) {
      ...
      if (typeof deltaContent === "string") streamedContent += deltaContent;
      if (typeof finalContent === "string") messageContent += finalContent;
    }
    return messageContent || streamedContent;
  }
...
function createChatApiClient(options = {}) {
  ...
  async function callChatApi(settings, messages, responseFormat, requestOptions = {}) {
    ...
    try {
      ...
      if (error.name === "AbortError") throw new Error("API 请求超时，请检查网络、模型或 Base URL 后重试。");
      throw error;
    } finally {

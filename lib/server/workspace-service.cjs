...
function createWorkspaceService({ buildDocumentKnowledgeModel, hasApiKey = () => false }) {
  ...
  function publicState(db) {
    const apiKeySet = Boolean(hasApiKey(db) || db.settings?.apiKey);
    const state = {
      ...db,
      settings: {
        ...db.settings,
        apiKey: apiKeySet ? "__SET__" : "",
      },
    };
